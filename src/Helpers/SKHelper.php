...
namespace Skato\SkatoAdmin\Helpers;
...
/**
 * Class skHelper
 * @package Skato\SkatoAdmin\Helpers
 *
 * This is SkatoAdmin Helper class contains methods required for Admin Panel functionality.
 */
class skHelper
{
    /**
     ...
     *
     * $names = skHelper::generateModuleNames($module_name);
     *
    ...
    /**
     * Get list of Database tables excluding SkatoAdmin Context tables like
     * backups, sk_configs, la_menus, migrations, modules, module_fields, module_field_types
     ...
     *
     * skHelper::parseValues($field['popup_vals']);
     *
     ...
     *
     * skHelper::log("info", "", $commandObject);
     *
    ...
    public static function recurse_copy($src, $dst)
    {
        ...
        while(false !== ($file = readdir($dir))) {
            if(($file != '.') && ($file != '..')) {
                if(is_dir($src . '/' . $file)) {
                    skHelper::recurse_copy($src . '/' . $file, $dst . '/' . $file);
                } else {
                    // ignore files
            ...
            foreach($objects as $object) {
                if($object != "." && $object != "..") {
                    if(is_dir($dir . "/" . $object))
                        skHelper::recurse_delete($dir . "/" . $object);
                    else
                        unlink($dir . "/" . $object);
                }
            }
     ...
     *
     * skHelper::createThumbnail($filepath, $thumbpath, $thumbnail_width, $thumbnail_height);
     *
     ...
     *
     * skHelper::print_menu_editor($menu)
     *
    ...
    {
        $editing = \Collective\Html\FormFacade::open(['route' => [config('skato-admin.adminRoute') . '.la_menus.destroy', $menu->id], 'method' => 'delete', 'style' => 'display:inline']);
        ...
        $childrens = \Skato\SkatoAdmin\Models\Menu::where("parent", $menu->id)->orderBy('hierarchy', 'asc')->get();
        
        if(count($childrens) > 0) {
            $str .= '<ol class="dd-list">';
            foreach($childrens as $children) {
                $str .= skHelper::print_menu_editor($children);
            }
            $str .= '</ol>';
        }
        $str .= '</li>';
    ...
    public static function print_menu($menu, $active = false)
    {
        ...
        $str = '<li' . $treeview . ' ' . $active_str . '><a href="' . url(config("skato-admin.adminRoute") . '/' . $menu->url) . '"><i class="fa ' . $menu->icon . '"></i> <span>' . skHelper::real_module_name($menu->name) . '</span> ' . $subviewSign . '</a>';
        ...
        if(count($childrens)) {
            ...
            foreach($childrens as $children) {
                ...
                if(Module::hasAccess($module->id)) {
                    $str .= skHelper::print_menu($children);
                }
            }
            $str .= '</ul>';
        }
     ...
     * This needs to be done recursively
     *
     * skHelper::print_menu_topnav($menu)
     *
     * @param $menu menu array from database
    ...
    public static function print_menu_topnav($menu, $active = false)
    {
        ...
        $str = '<li ' . $treeview . '' . $active_str . '><a ' . $treeview2 . ' href="' . url(config("skato-admin.adminRoute") . '/' . $menu->url) . '">' . skHelper::real_module_name($menu->name) . $subviewSign . '</a>';
        
        if(count($childrens)) {
            $str .= '<ul class="dropdown-menu" role="menu">';
            foreach($childrens as $children) {
                $str .= skHelper::print_menu_topnav($children);
            }
            $str .= '</ul>';
        }
        $str .= '</li>';
    ...
    public static function setenv($param, $value)
    {
        ...
        $envfile = skHelper::openFile('.env');
        $line = skHelper::getLineWithString('.env', $param . '=');
     ...
     *
     * skHelper::get_migration_file("students_table");
     *
