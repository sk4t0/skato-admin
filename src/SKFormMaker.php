...
class SKFormMaker
{
    ...
    public static function input($module, $field_name, $default_val = null, $required2 = null, $class = 'form-control', $params = [])
    {
        ...
        if(Module::hasFieldAccess($module->id, $module->fields[$field_name]['id'], $access_type = "write")) {
            ...
            if($unique && !isset($params['unique'])) {
                ...
                $params['adminRoute'] = config('skato-admin.adminRoute');
                if(isset($row)) {
                    $params['isEdit'] = true;
            ...
            switch($field_type->name) {
                ...
                case 'Dropdown':
                    ...
                    if($popup_vals != "") {
                        $popup_vals = SKFormMaker::process_values($popup_vals);
                    } else {
                        $popup_vals = array();
                    }
                ...
                case 'Taginput':
                    ...
                    $default_val = SKFormMaker::process_values($default_val);
     ...
     **/
    // $values = SKFormMaker::process_values($data);
        ...
        if(Module::hasFieldAccess($module->id, $module->fields[$field_name]['id'], $access_type = "view")) {
            ...
            switch($field_type->name) {
                ...
                case 'Decimal':
                    ...
                    break;
                case 'Dropdown':
                    $values = SKFormMaker::process_values($fieldObj['popup_vals']);
                    if(starts_with($fieldObj['popup_vals'], "@")) {
                        if($value != 0) {
                            $moduleVal = Module::getByTable(str_replace("@", "", $fieldObj['popup_vals']));
                            if(isset($moduleVal->id)) {
                                $value = "<a href='" . url(config("skato-admin.adminRoute") . "/" . $moduleVal->name_db . "/" . $value) . "' class='label label-primary'>" . $values[$value] . "</a> ";
                            } else {
                        ...
                        if(starts_with($fieldObj['popup_vals'], "@")) {
                            ...
                            foreach($values as $key => $val) {
                                if(in_array($key, $valueSel)) {
                                    $module_link = "";
                                    if(isset($moduleVal->id)) {
                                        $module_link = "href='" . url(config("skato-admin.adminRoute") . "/" . $moduleVal->name_db . "/" . $key) . "'";
                                    }
                ...
                case 'Taginput':
                    ...
                    if(count($values)) {
                        if(starts_with($fieldObj['popup_vals'], "@")) {
                            ...
                            foreach($values as $key => $val) {
                                if(in_array($key, $valueSel)) {
                                    $valueOut .= "<a href='" . url(config("skato-admin.adminRoute") . "/" . $moduleVal->name_db . "/" . $key) . "' class='label label-primary'>" . $val . "</a> ";
                                }
                            }
                        } else {
                            $valueSel = json_decode($value);
    ...
    public static function form($module, $fields = [])
    {
        ...
        foreach($fields as $field) {
            ...
            $out .= SKFormMaker::input($module, $field, null, null, 'form-control', array('id' => $field . '_' . $module->name));
        }
        return $out;
    }
