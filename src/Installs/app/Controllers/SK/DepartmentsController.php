...
class DepartmentsController extends Controller
{
	...
	public function index()
	{
		...
		if(Module::hasAccess($module->id)) {
			return View('sk.departments.index', [
	...
	public function store(Request $request)
	{
		if(Module::hasAccess("Departments", "create")) {
			...
			return redirect()->route(config('skato-admin.adminRoute') . '.departments.index');
	...
	public function show($id)
	{
		if(Module::hasAccess("Departments", "view")) {
			...
			if(isset($department->id)) {
				...
				return view('sk.departments.show', [
					'module' => $module,
	...
	public function edit($id)
	{
		if(Module::hasAccess("Departments", "edit")) {
			...
			if(isset($department->id)) {
				...
				return view('sk.departments.edit', [
					'module' => $module,
					'view_col' => $this->view_col,
				])->with('department', $department);
			} else {
		...
		for($i=0; $i < count($data->data); $i++) {
			for ($j=0; $j < count($this->listing_cols); $j++) { 
				...
				if($col == $this->view_col) {
					$data->data[$i][$j] =
                        '<a href="'
                        . url(config('skato-admin.adminRoute')
			...
			if($this->show_action) {
				...
				if(Module::hasAccess("Departments", "delete")) {
					$output .=
                        Form::open([
                            'route' => [config('skato-admin.adminRoute') . '.departments.destroy',
                            $data->data[$i][0]],
                            'method' => 'delete',
